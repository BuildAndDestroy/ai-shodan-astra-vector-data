...
def detect_location_from_query(query: str) -> list:
    """Extract location keywords from query.
    
    Returns list of country names that might be mentioned in the query.
    """
    query_lower = query.lower()
    
    # Location mappings - variations that might appear in queries
    locations = {
        "United States": ["united states", "usa", "us ", " us", "america", "american"],
        "Russia": ["russia", "russian", "rf", "moscow", "saint petersburg"],
        "Germany": ["germany", "german", "berlin"],
        "China": ["china", "chinese", "beijing"],
        "United Kingdom": ["united kingdom", "uk ", " uk", "britain", "british"],
...
def filter_results_by_location(results: list, target_locations: list) -> list:
    """Post-filter search results to prioritize specific locations.
    
    Moves results matching target locations to the front, maintaining order.
    """
    ...
    # Return matching results first, then non-matching
